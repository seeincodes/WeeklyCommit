...
/**
 * Temporary {@link NotificationSender} for non-prod environments where the real Resilience4j + DLT
 * client (group 7) isn't wired yet. Logs at WARN so its presence is visible; never silently absorbs
 * notifications.
 *
 * <p>Guarded by {@code @Profile("!prod")} so it cannot activate in production by accident — if a
 * prod deploy happens before group 7, the missing {@link NotificationSender} bean will fail context
 * load and crash the pod, which is the right failure mode.
 *
