...
- [ ] `DraftMode`: commit-create form (title input + `<RCDOPickerContainer />` + `<ChessTier />`-style tier picker + estimated hours + tags + submit) → `useCreateCommitMutation`; existing-commits list grouped by `<ChessTier />` with edit/delete/drag-reorder; "Lock Week" button → `useTransitionMutation({to:'LOCKED'})`; `<StateBadge state="DRAFT" />` in the header
- [ ] `LockedReadOnly`: read-only commit list grouped by `<ChessTier />`; `<StateBadge state="LOCKED" isReconcileEligible={false} />` with the "Reconciliation opens Friday" hint
- [ ] `ReconcileMode`: `<ReconcileTable commits={commits} onUpdate={patch}/>` wired to `useUpdateCommitMutation`; `<ReflectionField value={plan.reflectionNote} onChange={...}/>` wired to `useUpdateReflectionMutation` (debounced); "Submit reconciliation" button → `useTransitionMutation({to:'RECONCILED'})`; per-row + bulk `<CarryForwardRow />` / `<CarryAllButton />` wired to `useCarryForwardMutation`
- [ ] `ReconciledSummary`: read-only commit list with each row's `<CarryStreakBadge />`; full reflection note rendered (no edit); per-row + bulk `<CarryForwardRow />` (carry-forward stays available post-reconcile per [MVP6])
- [ ] `TeamPage`: `<TeamRollup rollup={useGetTeamRollupQuery({managerId, weekStart})} renderMember={(m) => <MemberCard ... onClick={openDrawer(m.employeeId)}/>}>`; drawer state held in URL search-params or a route param so deep-linking works
- [ ] `TeamMemberPage`: `<IcDrawer employeeId={params.employeeId} weekStart={...} onClose={navigate('/team')}/>` -- the addressable single-member view
